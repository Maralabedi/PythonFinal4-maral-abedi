...
def export_to_csv(students):
    ...
    open_file_safely(filename)
    
    abs_path = os.path.abspath(filename)
    messagebox.showinfo(
        "خروجی اکسل آماده شد ✅",
        f"فایل با موفقیت ذخیره شد.\n\nمسیر فایل:\n{abs_path}\n\n"
        f"می‌توانید آن را با اکسل یا Numbers باز کنید."
    )
...
<head>
    <meta charset="UTF-8">
    <title>کارنامه {student['first_name']} {student['last_name']}</title>
    <style>
...
<body>
    ...
    messagebox.showinfo(
        "کارنامه آماده شد ✅",
        f"کارنامه با موفقیت ساخته شد.\n\nمسیر فایل:\n{abs_path}\n\n"
        f"می‌توانید آن را با مرورگر باز کنید."
    )