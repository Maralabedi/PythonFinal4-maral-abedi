...
def open_file(filepath):
    """
    باز کردن فایل به صورت کراس‌پلتفرم (ویندوز، مک، لینوکس)
    """
    try:
        if sys.platform.startswith('darwin'):  # macOS
            ...
            print(f"نمی‌توان فایل را باز کرد: پلتفرم ناشناخته {sys.platform}")
    except Exception as e:
        from tkinter import messagebox
        messagebox.showerror("خطا در باز کردن فایل", f"نتوانستیم فایل را باز کنیم:\n{e}\nفایل در مسیر زیر ذخیره شد:\n{filepath}")
...
def export_to_csv(students):
    ...
    with open(filename, "w", encoding="utf-8-sig", newline="") as f:
        w = csv.writer(f)
        w.writerow(["آیدی", "نام", "نام‌خانوادگی", "سن", "ریاضی", "انگلیسی", "برنامه‌نویسی",
                    "دینی", "عربی", "فیزیک", "شیمی", "زیست", "معدل"])
        
        for s in students:
            g = [s['math'], s['english'], s['programming'], s['religion'],
                 s['arabic'], s['physics'], s['chemistry'], s['biology']]
            valid_grades = [x for x in g if x > 0]
...
<head>
    ...
    <title>کارنامه</title>
    <style>
        body{{font-family:Tahoma;background:linear-gradient(135deg,#667eea,#764ba2);padding:50px;}}
        ...
        .avg{{font-size:48px;color:#4CAF50;font-weight:bold;text-align:center;margin:40px;}}
    </style>
</head>
<body>
    <div class="card">
